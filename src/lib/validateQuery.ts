...
const validator = (input: Object) => {
    ...
    for (let i = 0; i < targetKey.length; i++) {
        // first if block can be reconfigured for different validation behavior based on a specific input
        if (targetKey[i] === inputArray[i][0] && targetKey[i] === specialKey) {
            const fileCheck = path.resolve(
                `./././assets/full/${inputArray[i][1]}.jpeg`
            )
